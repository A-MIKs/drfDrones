...
class DroneCategpryTests(APITestCase):
    ...
    def test_post_and_get_drone_category(self):
        #Ensure we create a new drone category and then retrieve it
    ...
    def test_filter_drone_category_by_name(self):
        ...
        url = "{0}?{1}".format(reverse(DroneCategoryList.name), urlencode(filter_by_name))
        print (url)
    ...
    def test_update_drone_category(self):
        ...
        url = reverse(
            DroneCategoryDetail.name, args = [response.data['pk']])
...
class PilotTests(APITestCase):
    ...
    def test_post_and_get_pilot(self):
        ...
        url = reverse(PilotDetail.name,  args=[saved_pilot.pk])
        ...
        assert unauthorized_get_response.status_code == status.HTTP_403_FORBIDDEN
